...
const users = ['使用者1', '使用者2', '使用者3']; // 借用者名單
        ...
        for (let day = 0; day < 6; day++) {
            ...
            if (status === '不可借用' || status === '每週不可借用') {
                ...
                if (isAdmin) {
                    ...
                    const options = ['可借用', '不可借用', '每週不可借用', ...users];
                    options.forEach(option => {
                        const opt = document.createElement('option');
                        opt.value = option;
                        opt.textContent = option;
                        if (status === option) opt.selected = true;
                        select.appendChild(opt);
                    });
                    select.addEventListener('change', () => updateSchedule(day, period - 1, select.value));
